...
| `madd.ew.x` | 933 | `ame:doc:inst:madd_ew_x` | Elementwise Arithmetic | `madd.ew.x md, xs1, ms2` | `D[i] = c + B[i]` | `ame-common-arithmetic` | - | Ready |
| `mand.ew` | 1026 | `ame:doc:inst:mand_ew` | Bitwise | `mand.ew md, ms1, ms2` | `D[i] = A[i] & B[i]` | `ame-common-arithmetic` | - | Ready |
| `mand.ew.x` | 1123 | `ame:doc:inst:mand_ew_x` | Bitwise | `mand.ew.x md, xs1, ms2` | `D[i] = c & B[i]` | `ame-common-arithmetic` | - | Ready |
| `mandnot.ew` | 1216 | `ame:doc:inst:mandnot_ew` | Bitwise | `mandnot.ew md, ms1, ms2` | `D[i] = A[i] & ~B[i]` | `ame-common-arithmetic` | - | Ready |
| `mandnot.ew.x` | 1315 | `ame:doc:inst:mandnot_ew_x` | Bitwise | `mandnot.ew.x md, xs1, ms2` | `D[i] = c & ~B[i]` | `ame-common-arithmetic` | - | Ready |
...
| `mcmpge.ew.x` | 1724 | `ame:doc:inst:mcmpge_ew_x` | Compare and Predication | `mcmpge.ew.x md, xs1, ms2` | `D[i] = (c >= B[i]) ? -1 : 0` | `ame-common-arithmetic` | - | Ready |
...
| `mcmplt.ew.x` | 1922 | `ame:doc:inst:mcmplt_ew_x` | Compare and Predication | `mcmplt.ew.x md, xs1, ms2` | `D[i] = (c < B[i]) ? -1 : 0` | `ame-common-arithmetic` | - | Ready |
| `mcolbcast.ew.x` | 2016 | `ame:doc:inst:mcolbcast_ew_x` | Permutation | `mcolbcast.ew.x md, ms1, xs1` | `D[i,j] = A[i,c]` | `ame-common-structural` | - | Ready |
...
| `mmean.ew.x` | 5017 | `ame:doc:inst:mmean_ew_x` | Elementwise Arithmetic | `mmean.ew.x md, xs1, ms2` | `D[i] = (c + B[i]) / 2` | `ame-common-arithmetic` | - | Ready |
| `mmin.ew` | 5110 | `ame:doc:inst:mmin_ew` | Elementwise Arithmetic | `mmin.ew md, ms1, ms2` | `D[i] = min(A[i], B[i])` | `ame-common-arithmetic` | - | Ready |
| `mmin.ew.x` | 5208 | `ame:doc:inst:mmin_ew_x` | Elementwise Arithmetic | `mmin.ew.x md, xs1, ms2` | `D[i] = min(c, B[i])` | `ame-common-arithmetic` | - | Ready |
...
| `mmove8.m.x` | 5622 | `ame:doc:inst:mmove8_m_x` | Register move / data conversion | `mmove8.m.x md, xs2, xs1` | `D.rm[pos, 8] = X[xs2][7:0]` | `ame-common-register-groups` | - | Ready |
| `mmove16.m.x` | 5705 | `ame:doc:inst:mmove16_m_x` | Register move / data conversion | `mmove16.m.x md, xs2, xs1` | `D.rm[pos, 16] = X[xs2][15:0]` | `ame-common-register-groups` | - | Ready |
| `mmove32.m.x` | 5788 | `ame:doc:inst:mmove32_m_x` | Register move / data conversion | `mmove32.m.x md, xs2, xs1` | `D.rm[pos, 32] = X[xs2][31:0]` | `ame-common-register-groups` | - | Ready |
| `mmove64.m.x` | 5871 | `ame:doc:inst:mmove64_m_x` | Register move / data conversion | `mmove64.m.x md, xs2, xs1` | `D.rm[pos, 64] = X[xs2][63:0]` | `ame-common-register-groups` | - | Ready |
| `mmove8.x.m` | 5960 | `ame:doc:inst:mmove8_x_m` | Register move / data conversion | `mmove8.x.m xd, ms2, xs1` | `X[xd] = zero_extend(A.rm[pos, 8])` | `ame-common-register-groups` | - | Ready |
...
| `mmulacc.ew.x` | 6801 | `ame:doc:inst:mmulacc_ew_x` | Elementwise Arithmetic | `mmulacc.ew.x md, xs1, ms2` | `D[i] = D[i] + c × B[i]` | `ame-common-arithmetic` | - | Ready |
...
| `mmulsub.ew.x` | 8220 | `ame:doc:inst:mmulsub_ew_x` | Elementwise Arithmetic | `mmulsub.ew.x md, xs1, ms2` | `D[i] = c - B[i] × D[i]` | `ame-common-arithmetic` | - | Ready |
| `mor.ew` | 8317 | `ame:doc:inst:mor_ew` | Bitwise | `mor.ew md, ms1, ms2` | `D[i] = A[i] \\| B[i]` | `ame-common-arithmetic` | - | Ready |
| `mor.ew.x` | 8414 | `ame:doc:inst:mor_ew_x` | Bitwise | `mor.ew.x md, xs1, ms2` | `D[i] = c \\| B[i]` | `ame-common-arithmetic` | - | Ready |
| `mornot.ew` | 8507 | `ame:doc:inst:mornot_ew` | Bitwise | `mornot.ew md, ms1, ms2` | `D[i] = A[i] \\| ~B[i]` | `ame-common-arithmetic` | - | Ready |
| `mornot.ew.x` | 8606 | `ame:doc:inst:mornot_ew_x` | Bitwise | `mornot.ew.x md, xs1, ms2` | `D[i] = c \\| ~B[i]` | `ame-common-arithmetic` | - | Ready |
| `mpack.ew.x` | 8699 | `ame:doc:inst:mpack_ew_x` | Register move / data conversion | `mpack.ew.x md, ms1, xs1` | `D[k] = convert(A)` | `ame-common-register-groups` | - | Ready |
...
| `mrowbcast.ew.x` | 10100 | `ame:doc:inst:mrowbcast_ew_x` | Permutation | `mrowbcast.ew.x md, ms1, xs1` | `D[i,j] = A[c,j]` | `ame-common-structural` | - | Ready |
...
| `msublog2.ew.x` | 13145 | `ame:doc:inst:msublog2_ew_x` | Elementwise Math Functions | `msublog2.ew.x md, xs1, ms2` | `D[i] = c - log~2~(\\|B[i]\\|)` | `ame-common-arithmetic` | - | Ready |
| `mtanh.ew` | 13241 | `ame:doc:inst:mtanh_ew` | Elementwise Math Functions | `mtanh.ew md, ms1` | `D[i] = tanh(A[i])` | `ame-common-arithmetic` | - | Ready |
| `munpack.ew.x` | 13336 | `ame:doc:inst:munpack_ew_x` | Register move / data conversion | `munpack.ew.x md, ms1, xs1` | `D = convert(A[k])` | `ame-common-register-groups` | - | Ready |
| `mxor.ew` | 13430 | `ame:doc:inst:mxor_ew` | Bitwise | `mxor.ew md, ms1, ms2` | `D[i] = A[i] ^ B[i]` | `ame-common-arithmetic` | - | Ready |
| `mxor.ew.x` | 13527 | `ame:doc:inst:mxor_ew_x` | Bitwise | `mxor.ew.x md, xs1, ms2` | `D[i] = c ^ B[i]` | `ame-common-arithmetic` | - | Ready |
...
- After: each of the two operands is both a source and a destination: the instructions consume the sole square from each of `ms1` and `ms2` and write the two result squares back into `ms1` and `ms2`, so every square keeps its own operand alignment and no derived consecutive base remains.  `ms1` and `ms2` must name distinct M registers; an equal pair raises an `Illegal Instruction` exception before any register is read or written, because an aliased pair would assign every element position twice.  Packed datatypes are no longer supported by the zip/unzip family; every Permutation instruction requires a datatype no smaller than `AME_UNIT_DATATYPE_SIZE`.
